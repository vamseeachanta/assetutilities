...
class ReadDataFromString:
    ...
    def get_line_numbers_containing_keyword(self, cfg):
        ...
        line_numbers = self.get_row_numbers_containing_keyword(
    ...
    def get_substrings_by_line_number_array(self, cfg):
        ...
        if start_line_count == end_line_count:
            ...
            raise (
                f"End Line array/keyword is missing. Missing count is {start_line_count - end_line_count}"
            )
        else:
            raise (
                f"End Line array/keyword is missing. Missing count is {end_line_count - start_line_count}"
            )
    ...
    def get_all_lines_containing_all_key_words(self, cfg):
        ...
        line_numbers = self.get_all_array_row_containing_all_keywords(
    ...
    def get_all_lines_containing_any_key_words(self, cfg):
        ...
        key_words = cfg["line"].get("key_words", None)
        line_array_rows = data_string.split("\n")
        line_numbers = self.get_all_array_rows_containing_any_keyword(
            line_array_rows, key_words
        )
        selected_lines = [
            line_array_rows[line_number - 1] for line_number in line_numbers
...
class ReadURLData:
    ...
    def get_response_data(self, cfg):
        try:
            ...
            data = None
            logging.info(
                f"Failed to parse xml from response ({traceback.format_exc()})"
            )
