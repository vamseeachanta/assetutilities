        ...
        for file in cfg["files"]:
            ...
            for sheetname in file["sheet_name"]:
                for array_range in file["range"]:
                    ...
                    output_basename = (
                        io_basename + "_" + sheetname + "_" + array_range[0]
                        + array_range[1]
                    )
                    for ext in file["output_extension"]:
                        ...
                        excel2img.export_img(
                            io, output_filename, "", sheet_range
                        )
    ...
    def csv_copy_to_excel(self, cfg):
        ...
        for group in groups:
            ...
            for csv in csvs:
                ...
                if not is_file_valid:
                    logger.debug(
                        f"File {inputs_csv} not found. Skipping {target_file}"
                    )
                    logger.debug(f"Skipping {target_file}  .. FAIL")
                    if os.path.exists(target_file):
                        logger.info(
                            f"Removing {target_file} as it is not updated"
                        )
                    ...
                    logger.info(
                        f"   ... Copying data in {inputs_csv} to {worksheet}"
                    )
