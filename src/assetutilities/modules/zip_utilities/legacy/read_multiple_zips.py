...
class ReadMultipleZips:
    ...
    def router(self, cfg):
        if cfg["zip"]["files"] == "multiple":
            self.multiple_zips_to_dataframes(
                cfg["zip_folderpath"], cfg["column_names"]
            )
        return cfg

    def multiple_zips_to_dataframes(
        folder_path: str, column_names: Optional[List[str]] = None
        ...
        for zip_filename in zip_files:
            ...
            with zipfile.ZipFile(zip_filepath, "r") as zf:
                ...
                csv_or_txt_files = [
                    f for f in file_list if f.lower().endswith((".csv", ".txt"))
                ]
